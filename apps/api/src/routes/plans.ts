import { workoutPlanInputSchema } from '@fitflow/contracts';
import { db, exercises, workoutPlanDays, workoutPlanExercises, workoutPlans } from '@fitflow/db';
...
export const plansRoutes = new Hono<AppEnv>()
  ...
  .post('/', zValidator('json', workoutPlanInputSchema), async (c) => {
    const coach = c.get('user');
